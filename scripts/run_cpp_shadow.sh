...
echo "Starting Mission Commander, C++ Shadow Node, and Logger..."
docker compose run --rm -d --name cpp_shadow simulation bash -c "ros2 run precision_landing_control_cpp control_node --ros-args -r /precision_landing/cmd_vel:=/precision_landing/cpp_shadow_cmd -p kp_x:=0.002 -p kp_y:=0.002 -p flip_x:=true -p flip_y:=true"
docker compose run --rm -d --name shadow_logger simulation bash -c "ros2 run px4_vision_autonomy shadow_logger"
